...
if [[ "${internal_phoenix}" == "true" ]]; then
    ...
    #unclass=$(sed -n '2p' "${kraken2_asmbled_summary}" | cut -d' ' -f2 | xargs echo)
		...
		# If there are classified reads then check to see if percent unclassifed falls above the threshold limit. Report warning if too high or success and stats if below
			...
			elif (( $(echo "${genuspostpercent} < 70" | $bc_path -l) )); then
				printf "%-30s: %-8s : %s\\n" "KRAKEN2_CLASSIFY_ASMBLD" "WARNING" "Genus-${genuspost}(${genuspostpercent}%) is under 70% (species ${speciespost} (${speciespostpercent}%)), possibly contaminated or scaffolds are weighted unevenly"  >> "${sample_name}.synopsis"
				if [[ "${status}" = "SUCCESS" ]] || [[ "${status}" = "ALERT" ]]; then
					status="WARNING"
				fi
			else
...
if [[ -s "${kraken2_weighted_summary}" ]]; then
	...
	# If there are classified reads then check to see if percent unclassifed falls above the threshold limit. Report warning if too high or success and stats if below
		...
		elif (( $(echo "${genusweightedpercent} < 70" | $bc_path -l) )); then
			printf "%-30s: %-8s : %s\\n" "KRAKEN2_CLASSIFY_WEIGHTED" "FAILED" "Genus-${genusweighted} is under 70% (species-${speciesweighted} ${speciesweightedpercent}%), likely contaminated"  >> "${sample_name}.synopsis"
			status="FAILED"
		else
